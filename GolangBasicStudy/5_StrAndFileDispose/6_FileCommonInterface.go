/*
		文件常用操作接口:
	        创建文件对象:
	            ...
	            3. os.Mkdir(path string,perm int): 根据所给path创建文件夹, perm代表操作权限, 默认为0666
	        打开文件:
...
func main() {
	// 创建一个空文件
	file, err := os.Create("D:\\Users\\Administrator\\IdeaProjects\\Go\\GoStudy\\src\\1_Go语言基础学习\\5_文本文件处理\\" +
		"test.txt")
	if err != nil {
		fmt.Println(err)
		return
	...
	_, err_file := file.Write(append(data, '1', '2', '3'))
	if err_file != nil {
		fmt.Println(err_file)
		return
	}
	_, err_string := file.WriteString("你好, 我是混沌")
	if err_string != nil {
		fmt.Println(err_string)
		return
	}
...
func fileRead() {
	file_data := make([]byte, 10240, 10240)
	// 打开文件
	file, err_file := os.Open("D:\\Users\\Administrator\\Desktop\\小练习\\test\\蜘蛛侠.txt")
	if err_file == nil {
