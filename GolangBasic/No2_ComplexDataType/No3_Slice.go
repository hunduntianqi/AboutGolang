...
/*
   切片(slice):
       ...
       定义切片:
           ...
           sliceName: 切片名称
           type: 切片中存储元素的数据类型
			注意: 当type == interface{}时, 该切片就具备了存储任意类型数据的能力, 几乎等同于Python的list
       使用make()函数创建切片:
           var sliceName []type = make([]type, len, capacity)
           或简写为: sliceName := make([]type, len, capacity)
...
func deleteSpliceElement() {
	...
	// 使用切片方式删除第一个元素
	fmt.Println("使用切片方式删除第一个元素: ")
	list = list[1:] // 产型新的数组, 会产生一个新的切片, 与原切片指针不同
	fmt.Println(list)
	fmt.Printf("list ==> len = %d, cap = %d\n", len(list), cap(list))
	fmt.Println("list的内存地址为:", &list[0])
	// 使用append函数删除第一个元素
	fmt.Println("使用append函数删除第一个元素: ")
	list = append(list[:0], list[1:]...) // 不会改变原来切片的内存结构
	fmt.Println(list)
	fmt.Printf("list ==> len = %d, cap = %d\n", len(list), cap(list))
	fmt.Println("list的内存地址为:", &list[0])
	// 使用copy删除第一个元素
	fmt.Println("使用copy删除第一个元素: ")
	...
	fmt.Println("list的内存地址为:", &list[0])
	// 使用copy删除第4个元素
	list = list[:3+copy(list[3:], list[4:])]
	fmt.Println(list)
	fmt.Printf("list ==> len = %d, cap = %d\n", len(list), cap(list))
	fmt.Println("list的内存地址为:", &list[0])
