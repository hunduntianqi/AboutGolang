	...
	结构体:
		...
		结构体被定义后, 就可以像其他基本数据类型一样进行变量声明 (个人认为类似于Java中的创建对象)
            格式: variable_name := struct_name {member1_value, member2_value,...}
                ...
                或 variable_name := struct_name {key1:value1, key3:value3, ...}
				未初始化成员变量为基本类型默认值
		访问结构体成员:
            格式: 结构体名.成员名称
        修改结构体成员变量的值:
