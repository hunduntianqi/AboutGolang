...
func main() {
	...
	for {
		...
		for index, li := range nodes {
			...
			// 获取图片名称
			picName := htmlquery.InnerText(htmlquery.FindOne(li, "./a/@title")) + "_" + strconv.Itoa(page) + "_" + strconv.Itoa(index+1)
...
func getBigUrl(link string, client *http.Client, header map[string]string) string {
	...
	// 定义变量, 存储大图Url
	bigPicUrl := ""
	bigPic := htmlquery.FindOne(documnet, "/html/body/div[4]/img/@src")
	// 解析获取大图链接
	if bigPic != nil {
		bigPicUrl = htmlquery.InnerText(bigPic)
	}
	return bigPicUrl
}
